...
def groupCost(g): return len(g) * discount[len(g)]

class Grouping:
	def __init__(self, groups = [set()]):
		self.groups = groups
	def total(self): return sum(map(groupCost,self.groups)) * BASE_COST
	def dup(self): return Grouping(list(map(set,self.groups)))
	def add(self, b, index=0):
		other = self.dup()
		other.groups.sort(key=lambda g: len(g))
		valid = list(filter(lambda g: b not in g, other.groups))
		if len(valid) == 0: other.groups.append(set([b]))
		else: valid[index].add(b)
		return other
